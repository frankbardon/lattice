...
| [`examples/page-dashboard.json`](https://github.com/frankbardon/lattice/blob/main/examples/page-dashboard.json) | A page-like document of block-wrapped [content leaves](../format/content.md) — a `heading`, two `markdown` paragraphs, and a captioned `image` — with `${var}` interpolation into the heading and prose. |
