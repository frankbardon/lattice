...
| [`examples/dropdown-dashboard.json`](https://github.com/frankbardon/lattice/blob/main/examples/dropdown-dashboard.json) | The live runtime-input loop: a dropdown sets a variable consumed by a table. |
...
variables; `$var` typed bindings and `${}` string templates; a dropdown runtime
input; both connection types; a `$secret` reference that is redacted from the
...
The dropdown and kitchen-sink examples are most interesting under `serve`, where
