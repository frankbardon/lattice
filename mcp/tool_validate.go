...
// The validate_patch tool defined here is the SIMULATE step of the
// propose-then-commit contract: given a document id and the model's cumulative
// id-rooted RFC 6902 changeset, it runs the dry-run primitive
...
// pipeline's *errors.CodedError verbatim as a tool error so the model can
// self-correct and re-validate.
//
// Like the other tools it calls ONLY the ./service facade and surfaces coded
// errors verbatim, never as a flattened plain string.
...
// validatePatchDescription is the validate_patch tool description, kept identical
// to the legacy registration so downstream catalog text (get_manifest) holds
// parity.
const validatePatchDescription = "Simulate an id-rooted RFC 6902 JSON Patch against a dashboard WITHOUT saving. This is the validate-only step of the propose-then-commit contract: it runs the full apply+re-resolve pipeline under every guardrail but NEVER persists — there is no save path through MCP. Resend the FULL cumulative patch each call (the server is stateless). On success returns {ok:true} with the resolved preview and the document's baseRevision; on failure returns {ok:false} plus the coded error (code + details) as a tool error so you can correct the ops and re-validate. To persist a validated patch, the human commits it via POST /api/patch, passing baseRevision as expectedRevision."

// validatePatch parses the input ops into a *service.Changeset
// (service.ParseChangeset), runs the dry-run primitive (service.DryRunPatch) —
// which loads, resolves, applies, and re-resolves under every guardrail but NEVER
// saves — and, on success, returns the resolved preview plus the document's
// current base revision. A parse failure (PATCH_*), an off-surface/ill-typed/
// structural changeset, or a re-resolution failure (STORAGE_NOT_FOUND,
// RESOLVE_*/SCHEMA_*/VAR_*) surfaces the *errors.CodedError verbatim as a tool
// error so the model can self-correct. The store is touched only by the dry-run's
// read-only Load and the revision lookup — never written.
func validatePatch(_ context.Context, svc *service.Service, in validatePatchInput) (validatePatchOutput, error) {
	...
	opsBytes, err := json.Marshal(in.Ops)
	if err != nil {
		return validatePatchOutput{OK: false}, err
	}
	...
	result, err := svc.DryRunPatch(in.ID, cs)
	if err != nil {
		return validatePatchOutput{OK: false}, err
	}
