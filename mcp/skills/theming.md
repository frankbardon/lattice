---
name: theming
description: How a lattice document expresses presentation as renderer-agnostic semantic theme tokens — the closed token vocabulary, the two independent layers (document default theme + per-block override), the no-merge "side-by-side" rule, and how lattice_get_outline's document-scope `theme` boolean flags presence (not values). Grammar (the exact token enums) stays in lattice_get_schema {type: "dashboard"}.
type: guide
kind: workflow
...
grammar drifts with the schema and is `lattice_get_schema`'s job (see **Reading the
grammar** below).
...
`lattice_get_outline`'s document-scope summary carries a **`theme` boolean**, not the
...
- `theme: true` → a default theme exists; read its actual tokens from the
  document scope (e.g. via `lattice_get_document` or by inspecting `$theme`).
...
`lattice_get_node {id, nodeId}` (its `subtree` carries `config.theme`; `theme` is on the
...
envelope's `theme` member**. The theme schema is not a standalone `lattice_get_schema`
...
- **`lattice_get_node {id, nodeId}`** on a block wrapper — its `surface` lists `theme`
  as a settable field; the surface (not this skill) is authoritative for which
  tokens are legal to patch.

Author or edit a theme, then prove it with **`lattice_validate_patch`** before a human
...
- **patch-authoring** — the `$theme` scope and id-rooted pointer dialect for
  editing tokens.
- **session-bootstrap** — why token enums stay in `lattice_get_schema`, not here.
