...
description: Read this FIRST. Defines the lattice MCP skill pack — its frontmatter schema, the source-layering doctrine (which tool is authoritative for what), the CALL-FIRST lattice_get_manifest flow, and the skill-file naming convention. Every other skill assumes you have read this.
type: guide
kind: workflow
applies_to: [lattice_get_manifest, lattice_list_skills, lattice_get_skill]
---
...
## CALL FIRST: `lattice_get_manifest`

Begin every session by calling **`lattice_get_manifest`**. It is the single orienting
...
1. **`lattice_get_manifest`** — orient: what server, what documents, what item types,
   what skills exist.
2. **`lattice_list_skills`** / **`lattice_get_skill`** — pull the skill(s) whose `applies_to`
   matches the tool(s) you are about to use.
3. The live read tools (`lattice_get_outline`, `lattice_get_node`, `lattice_get_schema`) — fetch the
...
tool will tell you. Critically, **a skill never re-emits `lattice_get_schema` grammar** —
...
|---|---|---|
| `lattice_get_schema` (+ `lattice_list_schemas`) | **Grammar** — the JSON Schema for an item type: its fields, types, enums, required keys | You need to know what a node's config may legally contain |
| `lattice_get_outline` | **Structure** — the document's node skeleton: ids, types, nesting, and a node's freeform `metadata` where it carries it | You need to navigate or locate a node |
| `lattice_get_node` | **Editable surface** — a single node's current config + which fields are settable | You are about to edit one node and need its present state |
| `lattice_validate_patch` | **Truth / simulate** — applies a proposed RFC 6902 changeset in a dry run and reports the resolved result or the coded error; never persists | You want to confirm an edit is valid *before* it touches anything |
...
- **Schemas are never copied into skills.** Ask `lattice_get_schema`. If a skill seems to
...
- **MCP never writes.** Every MCP tool is read-only or simulate-only.
  `lattice_validate_patch` *simulates* a change and shows you the outcome, but it persists
...
- **`lattice_validate_patch` is the proof step.** Before suggesting a change be written,
  run it through `lattice_validate_patch` and report what it resolved to (or the coded
  error it produced). Do not claim an edit is correct on inspection alone.
- **The catalog is open.** The item types `lattice_get_schema` knows are not a fixed
  ...
  `lattice_get_manifest` / `lattice_list_schemas` like any built-in. Never assume the catalog from
...
|---|---|---|---|
| `name` | yes | identifier | Stable name; the `lattice_get_skill` argument. Matches the file stem. |
| `description` | yes | one line | Summary shown in `lattice_list_skills` / `lattice_get_manifest` so a host can pick without fetching the body. |
...
| `applies_to` | yes | list | MCP tool names (or flows) this skill is relevant to, e.g. `[lattice_get_node, lattice_validate_patch]`. |
...
- The stem is the `lattice_get_skill` argument — callers pass the name without the `.md`
  extension.
...
- Keep grammar OUT of the file. If you find yourself transcribing a schema, stop
  and point the reader at `lattice_get_schema` instead.
