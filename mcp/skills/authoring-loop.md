---
name: authoring-loop
description: The end-to-end edit loop against a lattice dashboard — orient (get_manifest) → locate (get_outline → get_node) → read grammar (get_schema) → build an id-rooted RFC 6902 patch → simulate (validate_patch) → hand off for commit (POST /api/patch). Names which tool answers which question and when to loop back. Read patch-authoring for the patch grammar itself.
type: guide
kind: workflow
applies_to: [get_manifest, list_dashboards, get_outline, get_node, list_schemas, get_schema, validate_patch]
---
...
the CALL-FIRST `get_manifest` rule). This skill sequences the live tools; it does
...
read-only or simulate-only. You drive the loop up to a green `validate_patch`,
...
|---|---|---|
| 1. Orient | `get_manifest` | What server/version, which documents, what item types, which skills? |
| 2. Pick | `list_dashboards` | Which document id is my target? (id + title) |
| 3. Locate | `get_outline {id}` | Where is the node? Its id, type, nesting, the doc-scope summary, and the `revision`. |
| 4a. Drill (edit existing) | `get_node {id, nodeId}` | The node's stored `subtree` + its editable field `surface` — which field paths a patch may touch. |
| 4b. Discover (build new) | `list_schemas` → `get_schema {type}` | The full config grammar a NEW node/document of that type must satisfy. |
| 5. Build | — | Author the cumulative id-rooted RFC 6902 ops (see **patch-authoring**). |
| 6. Simulate | `validate_patch {id, ops}` | Is the patch valid? Returns `{ok, preview, baseRevision}` or a coded error. Iterate. |
| 7. Commit (human) | `POST /api/patch` | The only write. Out of band; you do not call it. |

### 1–2. Orient and pick

`get_manifest` first, always. It is the ground truth for *this* server — the
document ids, the item-type catalog, the skills index. Then `list_dashboards`
narrows to the `id` you will work on (every other tool keys off that id).

### 3. Locate — `get_outline`

`get_outline {id}` returns a config-free skeleton: each node's `id`, `type`,
...
`revision` for the eventual commit. Prefer the outline over `get_document`: the
...
- **Editing a node that already exists → `get_node {id, nodeId}`.** It returns
  ...
  patch. The surface **gates field edits** — `validate_patch` rejects a field op
...
- **Building a new node/document → `get_schema {type}`** (after `list_schemas`
  to discover the legal type tokens). The schema is the full config grammar a new
  node must satisfy. Do NOT read field grammar from this skill — `get_schema` is
  authoritative and drifts per server/type.

The surface (`get_node`) gates **field** edits only. **Structural** edits
(add/remove/move children) are NOT surface-listed — plan those from `get_outline`
(step 3). See **patch-authoring** for the field-vs-structural split.

### 5–6. Build and simulate — `validate_patch`

Build the cumulative id-rooted RFC 6902 patch, then call `validate_patch {id,
...
`validate_patch` is the proof step. Iterate it until `ok: true` before you hand
anything off.

#### When to loop back

- Off-surface field op (`CONFIG_OVERRIDE_FIELD_UNKNOWN`) → re-read `get_node`
...
- Ill-typed value (`CONFIG_OVERRIDE_VALUE_INVALID`) → re-check the surface field's
  `type` (step 4a) or `get_schema` (step 4b).
- Unknown node id (`CHANGESET_TARGET_NOT_FOUND`) → re-`get_outline` (step 3); the
...
- Structural reject (grammar/schema on re-resolve) → re-plan placement against
  `get_outline` (step 3) and the type's `get_schema` (step 4b).
...
against the new revision. Your last action is a successful `validate_patch`.

## Cross-links

- **session-bootstrap** — the doctrine + the CALL-FIRST `get_manifest` rule.
